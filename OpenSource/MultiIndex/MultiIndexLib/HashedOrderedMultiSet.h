...
public:
    ...
    static
    std::pair<I, I>
    EqualKeys(const typename BaseType::Bucket& bucket, const K& key, const Pred& pred) noexcept;
    ...
    BucketInsert(typename BaseType::Bucket& bucket, const Iter& key, const Pred& pred) noexcept;
    
    template<typename I, typename K>
    static I Find(const typename BaseType::Bucket& bucket, const K& key, const Pred& pred) noexcept;
private:
...
protected:
    HashedOrderedMultiSet(TupleParams<Pred>&& params) noexcept;
