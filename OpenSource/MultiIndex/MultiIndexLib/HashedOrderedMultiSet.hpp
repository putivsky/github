...
HashedOrderedMultiSet<Capacity, Iter, Pred>::EqualKeys(const typename BaseType::Bucket& bucket, const K& key, const Pred& pred) noexcept {
    auto lower = LowerBound<I, K>(bucket, key, pred);
    auto upper = std::upper_bound((Iter*)lower, bucket.m_head + bucket.m_size, key,
...
template<typename I, typename K>
/*static*/
I HashedOrderedMultiSet<Capacity, Iter, Pred>::LowerBound(const typename BaseType::Bucket& bucket, const K& key, const Pred& pred) noexcept {
